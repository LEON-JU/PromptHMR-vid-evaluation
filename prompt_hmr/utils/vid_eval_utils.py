# Modified from GVHMR: gvhmr/hmr4d/utils/eval/eval_utils.py
...
@torch.no_grad()
def compute_global_metrics(batch, mask=None, chunk_length=100):
    ...
    Returns:
        global_metrics (dict) containing W-MPJPE/WA-MPJPE (100 frame chunks),
        root trajectory error and optional motion diagnostics.
    """
    ...
    for start in range(0, seq_length, chunk_length):
        ...
        w_mpjpe100.append(compute_jpe(target_j3d, w_j3d))
        wa_mpjpe100.append(compute_jpe(target_j3d, wa_j3d))

    # Metrics
    m2mm = 1000
    w_mpjpe100 = np.concatenate(w_mpjpe100) * m2mm
    wa_mpjpe100 = np.concatenate(wa_mpjpe100) * m2mm
    ...
    global_metrics = {
        "w_mpjpe100": w_mpjpe100,
        "wa_mpjpe100": wa_mpjpe100,
        "rte": rte,
        "jitter": jitter,
